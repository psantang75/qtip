...
| `RATE_LIMIT_MAX_REQUESTS`  | number   | `1000`                                       | `1000`                                | api limiter (per-user/IP; dashboards fire 10-30 GETs) |
...
| `PHONE_RECORDING_BASE_PATH` | Optional rewrite for `tblConversationRecording.RecordingPath` before the `/api/phone-system/audio/:recordingId` streaming endpoint opens the MP3. Leave blank on Windows (the stored UNC path is read directly). On Linux, mount each recording share and set this to the local mount root(s). **Accepts a comma-separated list** because PhoneSystem writes recordings to more than one share over time — e.g. the legacy flat store `\\wagoneer\DMCMS\PhoneSystem Recording\<id>.mp3` (mount at `/mnt/qtip-audio`) and the current tree `\\F350\Divisions\Recordings\Phone System\Prod\<id>.mp3` (mount at `/mnt/recordings`), giving `PHONE_RECORDING_BASE_PATH=/mnt/qtip-audio,/mnt/recordings`. The leading UNC root is rewritten against each mount (with `/` separators, preserving the sub-path) and the first candidate that exists on disk is streamed. |
...
| `DESKTIME_API_KEY` `DESKTIME_BASE_URL` `DESKTIME_TIMEOUT_MS` `DESKTIME_MAX_RETRIES` | Read-only DeskTime client (company API key from DeskTime → Settings → API). Feeds each agent's daily `productiveTime` into the Productivity report's **Desk Utilization** column (`(productive desk time + Genesys "In Warehouse" time) / paid time`). `DESKTIME_BASE_URL` defaults to `https://desktime.com/api/v2/json`. The **DeskTime Activity** row on Admin → Insights → Report Schedules (`ie_source_report.report_code = desktime_daily`, default every 1h, re-pulling `incremental_days` = 2 ET days) loads `ie_fact_desktime_daily` via the source dispatcher; the roster reads that table first (live API only for days not yet stored). After first enabling the key on a box, backfill history once: `node backend/dist/workers/run-desktime-backfill.js 2026-03-01 <today>` (idempotent; DeskTime data starts 2026-03-02). When the key is unset, scheduled runs fail with an ingestion alert (deactivate the row on boxes without DeskTime), Desk Utilization shows as unavailable and the rest of the roster is unaffected. The DeskTime account's timezone must be Eastern so its calendar day matches the punch/Genesys day. |
