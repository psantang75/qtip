...
export const getForms = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[FORM CONTROLLER] Error in getForms:', error);
...
export const getFormById = async (req: Request, res: Response) => {
  try {
    logger.info('[FORM CONTROLLER] Getting form by ID');
    logger.info('[FORM CONTROLLER] Request params:', req.params);
    logger.info('[FORM CONTROLLER] Request query:', req.query);
    logger.info('[FORM CONTROLLER] User:', req.user);
    
    const form_id = parseInt(req.params.id);
    
    if (isNaN(form_id) || form_id <= 0) {
      logger.error('[FORM CONTROLLER] Invalid form ID:', req.params.id);
    ...
    logger.error('[FORM CONTROLLER] Error in getFormById:', error);
    
    if (error instanceof FormServiceError) {
      logger.error('[FORM CONTROLLER] FormServiceError:', {
    ...
    logger.error('[FORM CONTROLLER] Unexpected error:', error);
...
export const createForm = async (req: Request, res: Response) => {
  try {
    logger.info('[FORM CONTROLLER] Creating new form');
    logger.info('[FORM CONTROLLER] Request body keys:', Object.keys(req.body));
    logger.info('[FORM CONTROLLER] Form data size:', JSON.stringify(req.body).length, 'characters');
    ...
    logger.info('[FORM CONTROLLER] Starting form creation process...');
    const result = await formService.createForm(formData, created_by);
    
    logger.info('[FORM CONTROLLER] Form creation successful:', result);
    return res.status(201).json(result);
  } catch (error) {
    if (error instanceof FormServiceError) {
      logger.error('[FORM CONTROLLER] FormServiceError:', error.message, 'Code:', error.code);
      return res.status(error.statusCode).json({ 
        error: error.message,
        code: error.code 
      });
    ...
    logger.error('[FORM CONTROLLER] Error in createForm:', error);
    return res.status(500).json({ 
      error: 'Internal server error',
...
export const updateForm = async (req: Request, res: Response) => {
  try {
    logger.info('[FORM CONTROLLER] Updating form');
    logger.info('[FORM CONTROLLER] Form ID:', req.params.id);
    logger.info('[FORM CONTROLLER] Request body keys:', Object.keys(req.body));
    logger.info('[FORM CONTROLLER] Form data size:', JSON.stringify(req.body).length, 'characters');
    ...
    logger.info('[FORM CONTROLLER] Starting form update process...');
    const result = await formService.updateForm(form_id, formData, updatedBy);
    
    logger.info('[FORM CONTROLLER] Form update successful:', result);
    return res.status(200).json(result);
  } catch (error) {
    if (error instanceof FormServiceError) {
      logger.error('[FORM CONTROLLER] FormServiceError:', error.message, 'Code:', error.code);
    ...
    logger.error('[FORM CONTROLLER] Error in updateForm:', error);
    return res.status(500).json({ 
      error: 'Internal server error',
...
export const deactivateForm = async (req: Request, res: Response) => {
  try {
    logger.info('[FORM CONTROLLER] Deactivating form');
    
    const form_id = parseInt(req.params.id);
    const updatedBy = req.user?.user_id;
    if (!updatedBy) return res.status(401).json({ error: 'Unauthorized', code: 'UNAUTHORIZED' });

    logger.info('[FORM CONTROLLER] Deactivation request details:', {
      form_id,
      updatedBy,
      user: req.user,
      hasAuth: !!req.user,
    ...
    logger.info('[FORM CONTROLLER] Deactivation successful:', result);
    return res.status(200).json(result);
  } catch (error) {
    if (error instanceof FormServiceError) {
      logger.error('[FORM CONTROLLER] FormServiceError in deactivateForm:', {
        message: error.message,
        code: error.code,
        statusCode: error.statusCode,
    ...
    logger.error('[FORM CONTROLLER] Unexpected error in deactivateForm:', {
      error: error,
