...
export async function finalizeSubmission(submissionId: number, userId: number): Promise<FinalizeResult> {
  ...
  // NOTE: the agent/manager "review done" email fires at SUBMIT time
  // (see SubmissionService.submitAudit / promoteDraftToSubmitted →
  // notifySubmissionGraded), not here. By the time a submission is
  // finalized the CSR has already been notified, so finalizing is silent.
