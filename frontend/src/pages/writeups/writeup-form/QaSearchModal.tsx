...
function AnswerPicker({ question, value, onChange }: { question: FormQuestion; value: string; onChange: (v: string) => void }) {
  ...
  if (question.question_type === 'RADIO' || question.question_type === 'MULTI_SELECT') {
    const options: RadioOption[] = question.radio_options ?? []
    return <div className="flex flex-wrap gap-2 mt-2">{options.map(o => chip(o.option_text, o.option_value))}{question.is_na_allowed && chip('N/A','N/A')}</div>
  }
  if (question.question_type === 'N_A') return <div className="flex gap-2 mt-2">{chip('N/A','N/A')}</div>
  return null
}
...
export function QaSearchModal({ csrId, onImport, onClose }: QaSearchModalProps) {
  ...
  const [rawResults, setRawResults] = useState<QaSearchResult[]>([])
  const [selected, setSelected]     = useState<Set<number>>(new Set())

  const { data: formsData } = useQuery({
    queryKey: ['forms-for-qa-search'],
    queryFn:  () => api.get('/forms?limit=200&is_active=true').then(r => r.data?.forms ?? r.data?.data?.items ?? r.data?.data ?? []),
    staleTime: Infinity,
  })
  const forms: FormOption[] = formsData ?? []
  ...
  const searchableCategories = useMemo((): FormCategory[] => {
    if (!formDetail?.categories) return []
    return (formDetail.categories as FormCategory[])
      .map(cat => ({ ...cat, questions: (cat.questions ?? []).filter(q => !EXCLUDED_TYPES.includes(q.question_type)) }))
  ...
  const grouped = useMemo<SubmissionRow[]>(() => {
    ...
    for (const r of rawResults) {
      if (!map.has(r.submission_id)) {
        map.set(r.submission_id, { submission_id: r.submission_id, submission_date: r.submission_date, interaction_date: r.interaction_date, form_name: r.form_name, matches: [] })
      }
      map.get(r.submission_id)!.matches.push({ category_name: r.category_name ?? null, question_text: r.question_text, answer: r.answer })
    }
  ...
  return (
    <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col gap-0 p-0">
      <DialogHeader className="px-5 pt-5 pb-3 border-b border-slate-100 shrink-0">
        ...
        <DialogDescription className="sr-only">Search QA submissions by form and question answer for the selected employee.</DialogDescription>
      </DialogHeader>
      ...
      {!csrId ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-2 py-12 px-6 text-center">
          ...
          <p className="text-[13px] text-slate-400 max-w-xs">Select an employee at the top of the write-up form before searching QA records.</p>
        </div>
      ) : (
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          {formId && (
            <div className="px-5 py-3 border-b border-slate-100 shrink-0 max-h-[280px] overflow-y-auto">
              {loadingForm ? <p className="text-[13px] text-slate-400 py-2">Loading questions…</p>
                : searchableCategories.length === 0 ? <p className="text-[13px] text-slate-400 py-2">No searchable questions in this form.</p>
                : (
                <>
                  <p className="text-[11px] text-slate-400 mb-2">Select one or more questions — results will include audits matching <span className="font-semibold text-slate-600">any</span> selection.</p>
                  <div className="space-y-2">
                    {searchableCategories.map(cat => (
                      <div key={cat.id} className="border border-slate-100 rounded-lg overflow-hidden">
                        <button type="button" className="w-full flex items-center justify-between px-3 py-2 bg-slate-50 hover:bg-slate-100 text-left transition-colors" onClick={() => toggleCat(cat.id)}>
                          <span className="text-[12px] font-semibold text-slate-600">{cat.category_name}</span>
                          {expandedCats.has(cat.id) ? <ChevronDown className="h-3.5 w-3.5 text-slate-400" /> : <ChevronRight className="h-3.5 w-3.5 text-slate-400" />}
                        </button>
                                ...
                                <div key={q.id} className={cn('px-3 py-2.5 transition-colors', isSelected ? 'bg-primary/5' : 'hover:bg-slate-50')}>
                                  ...
                                  {isSelected && (
                                    <div className="mt-2 pl-6">
                                      ...
                                      <AnswerPicker question={q} value={qFilter?.answerValue ?? ''} onChange={v => setAnswer(q.id, v)} />
                                    </div>
                                  )}
                                </div>
                              )
          ...
          <div className="px-5 py-3 shrink-0 border-b border-slate-100">
            <Button size="sm" className="bg-primary hover:bg-primary/90 text-white" onClick={() => searchMut.mutate()}
              disabled={!canSearch || searchMut.isPending} title={!canSearch ? 'Select a form and at least one question with an answer' : undefined}>
              <Search className="h-3.5 w-3.5 mr-1.5" />{searchMut.isPending ? 'Searching…' : 'Search'}
            </Button>
            {!canSearch && formId && (
              <p className="text-[11px] text-slate-400 mt-1.5">
                {filters.length === 0 ? 'Select one or more questions above' : completeFilters.length === 0 ? 'Select an answer for at least one question' : ''}
              </p>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {grouped.length === 0 ? (
              <p className="text-[13px] text-slate-400 py-6 text-center">{searchMut.isIdle ? 'Set your criteria above and search' : 'No matching submissions found'}</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {grouped.map(r => (
                    <TableRow key={r.submission_id} className="cursor-pointer" onClick={() => toggleSel(r.submission_id)}>
                      <TableCell><Checkbox checked={selected.has(r.submission_id)} onCheckedChange={() => toggleSel(r.submission_id)} onClick={e => e.stopPropagation()} /></TableCell>
                      <TableCell className="text-slate-500 whitespace-nowrap text-[12px] align-top pt-3">{formatQualityDate(r.interaction_date ?? r.submission_date)}</TableCell>
                      <TableCell className="align-top pt-2.5">
                        <div className="space-y-1">
                          {r.matches.map((m, i) => (
                            <div key={i} className="text-[12px]">
                              {m.category_name && <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mr-1.5">{m.category_name}</span>}
                      ...
                      <TableCell className="text-center align-top pt-3">
                        <a href={`/app/quality/submissions/${r.submission_id}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="inline-flex text-primary hover:text-primary/70 transition-colors" title="View completed form">
          ...
          <div className="shrink-0 flex items-center justify-between px-5 py-3 border-t border-slate-100">
            <span className="text-[13px] text-slate-500">{selected.size > 0 ? `${selected.size} audit${selected.size !== 1 ? 's' : ''} selected` : 'No audits selected'}</span>
            <Button size="sm" className="bg-primary hover:bg-primary/90 text-white" disabled={selected.size === 0} onClick={handleImport}>Add to Write-Up</Button>
          </div>
        </div>
      )}
    </DialogContent>
