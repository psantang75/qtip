...
export default function AAProductivityPage() {
  ...
  const { data: rows, isLoading, isError } = useQuery({
    queryKey: ['aa-productivity', filters.params.period, filters.params.start, filters.params.end, filters.params.users, filters.params.departments],
  ...
  return (
    ...
    >
      {/* No Daily Trend here: it reads the segment-less 8am bucket
          ...
          other. The CSR Productivity page keeps its trend. */}
      {isLoading ? (
        ...
        <InsightsSection title="Productivity by Agent">
          <p className="text-sm text-danger text-center py-6">Couldn't load productivity. Refresh to try again.</p>
        </InsightsSection>
      ) : (
        <>
          <InsightsSection title="Contact Manager">
            <TicketProductivityTable rows={cmRows} agentLabel="Salesperson" />
          </InsightsSection>
          <InsightsSection title="All Other Tickets & Tasks">
            <TicketProductivityTable rows={otherRows} agentLabel="Salesperson" />
          </InsightsSection>
        </>
      )}
    </ActivityReportShell>
