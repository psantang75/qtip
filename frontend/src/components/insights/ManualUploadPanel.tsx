...
/**
 * File preview + upload flow for a single import data type, plus its recent
 * history. Shared by the admin Manual Upload page (data type chosen by a
 * selector on that page) and the Email Feeds manual-upload drawer on Report
 * Schedules (data type fixed to the feed). Keeping one implementation avoids
 * two upload UIs drifting apart.
 */
export function ManualUploadPanel({ dataType, showHistory = true }: {
  dataType: string
  showHistory?: boolean
}) {
  ...
  const activeType = MANUAL_UPLOAD_TYPES.find(t => t.code === dataType)
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: (res: ImportResult) => {
      ...
      qc.invalidateQueries({ queryKey: ['ie-email-feeds'] })
    },
    onError: (err: Error) => toast({
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {file && <span className="text-[12px] text-slate-500 truncate">{file.name}</span>}
        </div>
